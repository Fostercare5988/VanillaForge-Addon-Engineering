...
| **NamPower** | [**Emyrk/nampower**](https://github.com/Emyrk/nampower) | `v4.6.2+` (Opt-in DLL) | Client-side spell-cast queueing (eliminates input latency), cooldown/aura/spell info (`SpellInfo`, `GetSpellNameAndRankForId`), binary combat event dispatches. |
...
- **Rule C16 & AP-32 Cooldown Framing & Timestamp Wrap Audit:** Enforces dual-epoch calculation for `GetActionCooldown` (WoWUIBugs #47), explicit `<Model>` frame elevation (`parent:GetFrameLevel() + 2`), and safe numeric alpha fallback chains.
