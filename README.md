...
| :--- | :--- | :--- | :--- |
| **ClassicAPI** | [**brues-code/ClassicAPI**](https://github.com/brues-code/ClassicAPI) | `v1.15.0+` (`v1.14.0+` baseline) | 570+ functions across ~60 modern retail-style `C_` namespaces (`C_Timer.After`/`NewTicker`, `UnitCastingInfo`/`UnitChannelInfo`, `C_NamePlate`, `C_UnitAuras` slot-batching, `FocusUnit`/`ClearFocus`, `C_Container` native coroutine sorting/queries, `C_Texture` atlas/spritesheets, `C_Map` 3-tier coordinates/waypoints, `C_EncodingUtil`, `C_GossipInfo`, `C_AddOns`), plus `hooksecurefunc`, `InCombatLockdown`, `table.wipe`, and an AST rewriter that compiles `#`, `%`, `string.match` on the Lua 5.0 VM. |
