...
12. **Cooldown Framing, 3D Model Layering & Epoch Wrapping (Rule C16 / AP-32):** In 1.12.1 under DXVK / system uptime drift, `GetActionCooldown` timestamps can exceed `GetTime()` (WoWUIBugs #47); always implement dual-epoch calculation (`start <= now`, floating-point drift clamp `< 1.0`, and 32-bit wrap calculation) to avoid bogus multi-day timers. Elevate cooldown text frame levels explicitly above parent buttons and `<Model>` frames (`parent:GetFrameLevel() + 2` or higher) with safe alpha inheritance (`button:GetAlpha() or parent:GetAlpha() or 1`) to prevent text occlusion or alpha crashes.
